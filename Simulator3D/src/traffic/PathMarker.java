...
public class PathMarker{
	...
	private float rotation; //rotation of car 
	...
	public PathMarker(int xtile, int ytile, float relativeX, float relativeY, int[] actions) {
		this.wsize = TileManager.wsize;
		this.tsize = TileManager.tsize;
		this.xtile = xtile;
		this.ytile = ytile;
		this.relativePos.x = relativeX;
		this.relativePos.y = relativeY;
		this.originalRelativePos = relativePos;
		this.xpos = xtile * tsize + relativeX * tsize - wsize/ 2;
		this.ypos = ytile * tsize + relativeY * tsize - wsize / 2;
		if(actions != null) {
	...
	//Vector from the center of the tile towards the PathMarker
	private Vector2f makeCenterVec() {
		Vector2f a = new Vector2f(0.5f * tsize, 0.5f * tsize);
		Vector2f b = new Vector2f(relativePos.x * tsize, relativePos.y * tsize);
	...
	private Vector2f getRelativePosByVector(Vector2f vec) {
		float relativeX = 0.5f + (vec.x / tsize);
		float relativeY = 0.5f + (vec.y / tsize);
	...
	private void updateWorldPosition() {
		this.xpos = xtile * tsize + relativePos.x * tsize - wsize / 2;
		this.ypos = ytile * tsize + relativePos.y * tsize - wsize / 2;
